...
**✅ WORKING:** Uses `mcp-remote` proxy to Linear's official hosted MCP server with OAuth 2.1 authentication. See [`LINEAR_MCP_SUCCESS.md`](../docs/mcp/setup/LINEAR_MCP_SUCCESS.md) for implementation details.
...
- Linear MCP uses **OAuth 2.1 authentication** (automatic via `mcp-remote`)
- First run will open your browser for authorization
- Tokens are cached locally for subsequent runs
- **No API key required** for MCP functionality

**Optional API Key Setup** (for direct Linear API access):
